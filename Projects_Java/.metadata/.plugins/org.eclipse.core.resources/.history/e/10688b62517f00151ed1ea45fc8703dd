...
public class Main {
	private static Command parse(String command){
		...
		final Command result = commander.get(split[1]);
		if (result == null){
	...
	private static Map <String, Command> commander = new HashMap<String, Command>();
	public static void main(String[] args){
		List<String> currently_loged_users = new ArrayList<String>();
		Map <String, Integer> mapper = new HashMap<String, Integer>(); 
		commander.put("login", new Login());
		commander.put("logout", new Logout());
		commander.put("shutdown", new Shutdown());
		commander.put("info", new Info());
